...
type ReceiptItemRequest struct {
	Amount       string   `json:"amount" mcp:"Item amount as string (e.g. '12.99') (required)"`
	Description  string   `json:"description" mcp:"Item description (required)"`
	CategoryId   *string  `json:"category_id,omitempty" mcp:"Category ID (overrides default_category)"`
	CategoryName *string  `json:"category_name,omitempty" mcp:"Category name (overrides default_category)"`
	BudgetId     *string  `json:"budget_id,omitempty" mcp:"Budget ID"`
	BudgetName   *string  `json:"budget_name,omitempty" mcp:"Budget name"`
	Tags         []string `json:"tags,omitempty" mcp:"Additional tags for this item (merged with receipt tags)"`
	Notes        *string  `json:"notes,omitempty" mcp:"Item-specific notes"`
}

// ReceiptStoreRequest represents a request to store a shopping receipt with multiple items
type ReceiptStoreRequest struct {
	StoreName           string               `json:"store_name" mcp:"Name of the store/shop (required, becomes destination expense account)"`
	ReceiptDate         string               `json:"receipt_date" mcp:"Date of the receipt in YYYY-MM-DD format (required)"`
	Items               []ReceiptItemRequest `json:"items" mcp:"Array of receipt items/transactions (required, at least one)"`
	TotalAmount         *string              `json:"total_amount,omitempty" mcp:"Expected total amount - if provided, validates that sum of items equals this"`
	SourceAccountId     *string              `json:"source_account_id,omitempty" mcp:"Source account ID for payment (use either source_account_id or source_account_name)"`
	SourceAccountName   *string              `json:"source_account_name,omitempty" mcp:"Source account name for payment (use either source_account_id or source_account_name)"`
	DefaultCategoryId   *string              `json:"default_category_id,omitempty" mcp:"Default category ID for items without category"`
	DefaultCategoryName *string              `json:"default_category_name,omitempty" mcp:"Default category name for items without category"`
	CurrencyCode        *string              `json:"currency_code,omitempty" mcp:"Currency code for all transactions (e.g. 'USD', 'EUR')"`
	Tags                []string             `json:"tags,omitempty" mcp:"Tags to apply to all transactions"`
	Notes               *string              `json:"notes,omitempty" mcp:"Notes for the receipt (applied to first transaction)"`
	ApplyRules          bool                 `json:"apply_rules" mcp:"Whether to apply processing rules (default: false)"`
	FireWebhooks        bool                 `json:"fire_webhooks" mcp:"Whether to fire webhooks (default: true)"`
}
